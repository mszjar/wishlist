...
describe("WishList", function () {
  ...
  describe('addToWishList', function() {
    ...
    it('should add a new item to the wish list', async function() {
      const name = "Car";
      const price = ethers.parseEther("3");
      await wishList.addToWishList(name, price);
      ...
      assert(wishListElement[0][4] === "0x0000000000000000000000000000000000000000"); // address(0)
    })
  })
  ...
  describe('buyItem', function() {
    ...
    it('should not buy an item if not enough funds are provided', async function() {
      const etherGiven = ethers.parseEther("1");
      await expect(wishList.connect(addr1).buyItem(owner.address, 1, { value: etherGiven })).to.be
      .revertedWithCustomError(wishList, "NotEnoughFunds");
    })

    it('should buy an item if enough funds are provided', async function() {
      ...
      assert(wishListElement[0][0] === 1n);
      assert(wishListElement[0][1] === 'Car');
    ...
    it('should not buy an item if it is already bought', async function() {
      const etherGiven = ethers.parseEther("3");
      await wishList.connect(addr1).buyItem(owner.address, 1, { value: etherGiven });
      await expect(wishList.connect(addr2).buyItem(owner.address, 1, { value: etherGiven })).to.be
      .revertedWithCustomError(wishList, "AlreadyBought");
    })
